...
public class ExtractGravelAndStones_ extends ImagePlus implements PlugIn  {
	...
	public void run(String arg) {
		...
		MenuWaiter.GravelExtractionOptions mGEO = menu.showGravelExtractionMenu();
		...
		mFC.myOutFolder = mFC.myBaseFolder + pathSep + "GravelNStones" + (int)mGEO.threshold; 	 	
		...
		for (i = 0 ; i < mFC.myTiffs.length ; i++) {  //myTiffs.length
			...
			//apply analyzes			
			ImagePlus outTiff = jIM.extractGravel(mFC, colRoi.nowTiff, mGEO);		
