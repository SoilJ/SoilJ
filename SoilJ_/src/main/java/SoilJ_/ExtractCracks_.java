...
public class ExtractCracks_ extends ImagePlus implements PlugIn  {
	...
	public void run(String arg) {
		...
		for (i = 0 ; i < mFC.myTiffs.length ; i++) {  //myTiffs.length
			...
			//apply analyzes			
			ImagePlus outTiff = jIM.extractCracks(mFC, colRoi.nowTiff, mCEO);		
						
			//save result
			jIO.tiffSaver(mFC, outTiff);
			
			//also save non-bio pores
			//jIO.tiffSaver2(mFC, jIM.extractNonBioPores(mFC, colRoi.nowTiff, outTiff, mCEO));	
			...
			IJ.showStatus("Cracks have been extracted.");
