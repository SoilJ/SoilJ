	...
	public void run(String arg) {
		...
		for (i = 0 ; i < mFC.myTiffs.length ; i++) {  
			...
			for (int j = 0 ; j < mWRC.tensionStepsInMM.length ; j++) {
				...
				if (mWRC.tensionStepsInMM[j] < 0) mFC.fileName = "AirNWaterAt_neg" + String.format("%04.0f",-1 * mWRC.tensionStepsInMM[j]) + "mm_" + fileName;
				else mFC.fileName = "AirNWaterAt" + String.format("%04.0f",mWRC.tensionStepsInMM[j]) + "mm_" + fileName;
